...
export default defineComponent({
        ...
        constructControls () {
            ...
            for (const key in AVAILABLE_SCALES) {
                this.htmControls[1].options.push({
                    ...
                    enabled: true,
                    label: key,
                    onclick: ['htm.set-scale', parseFloat(key) / 100],
                })
            }
            if (currentScale > -1) {
    ...
    mounted () {
        ...
        this.$store.state.addEventListener('set-active-resource', (event) => {
            if ((event.detail.payload as { modality: string })?.modality === 'htm') {
                this.$nextTick(() => {
