...
<script lang="ts">
/**
 * ACC recording plot. Derived from EegPlot — same WebGL renderer, same
 * cache-state pipeline, rebound to the `acc` scope. EEG-specific
 * controls (epoch mode, analysis-tool overlays, source localisation)
 * are stripped.
 */
...
import { useAccContext } from "#app/modules/acc"
import { AccNavigationKey } from "../types"
...
export default defineComponent({
    ...
    methods: {
                ...
                const [r, g, b, a] = !useRaw && chanType
                                    ? this.SETTINGS.trace.colorSides && chanType === 'acc'
                                      ? chan.laterality === 's' ? traceColor.sin
                                        : chan.laterality === 'd' ? traceColor.dex
                                        : chan.laterality === 'z' ? traceColor.mid
                                        : traceColor[chanType]
                                      : traceColor[chanType]
                                    : traceColor.default
        ...
        handleKeydown (event: KeyboardEvent) {
            ...
            const key = event.key as AccNavigationKey
    ...
    async mounted () {
        ...
        this.$nextTick(() => {
            ...
            // This happens with small files that finish loading before the
            // view component is mounted.
