...
export default defineComponent({
    ...
    setup (props) {
        ...
        const analysis = useBiosignalAnalysis(context.RESOURCE, plotSelections, 'EmgViewer')
        ...
        const nav = useBiosignalNavigation(
            ...
            visibleRange,
            (position, goTo) => {
                goTo(position - position%secPerPage.value)
            }
        )
        ...
        const keyboard = useBiosignalKeyboard(
            ...
            () => {
                store.dispatch('emg.set-cursor-tool', null)
                analysis.closeAnalysisWindow()
                contextMenu.value = null
                pointer.hideAllDragElements()
                store.dispatch('emg.set-open-sidebar', null)
