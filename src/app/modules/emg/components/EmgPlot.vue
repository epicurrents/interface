...
export default defineComponent({
    ...
    watch: {
        dimensions () {
            // Drawing the entire plot is expensive, so only do it when necessary.
    ...
    methods: {
        ...
        handleKeydown (event: KeyboardEvent) {
            ...
            const key = event.key as EmgNavigationKey
                ...
                if (key === 'ArrowUp') {
                    ...
                    for (let i=0; i<sensVals.length; i++) {
                        if (i && sensVals[i] >= this.RESOURCE.sensitivity) {
                            this.$store.dispatch('emg.set-sensitivity', sensVals[i-1])
                            return
                        }
                    }
                    ...
                    for (let i=0; i<sensVals.length; i++) {
                        if (sensVals[i] > this.RESOURCE.sensitivity) {
                            this.$store.dispatch('emg.set-sensitivity', sensVals[i])
                            return
    ...
    async mounted () {
        ...
        const newFrame = () => {
            if (this.RESOURCE.isAudioPlaying) {
                // Update view start to follow playback position.
                const audioPos = this.RESOURCE.playbackPosition
                if (audioPos >= 0 && audioPos < this.RESOURCE.totalDuration) {
                    const audioPage = audioPos - (audioPos - this.startOffset)%this.secPerPage
                    if (audioPage !== this.RESOURCE.viewStart) {
                        this.$emit('go-to', audioPage)
                    }
                }
            }
