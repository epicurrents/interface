...
export const schemas: InterfaceModuleSchema = safeObjectFrom({
    ...
    settings: {
        ...
        fields: [
            ...
            {
                text: 'Navigator options',
                type: 'subtitle',
                info: 'Options for the navigator strip below the signal view.',
            },
            {
                component: 'settings-checkbox',
                height: '2.5rem',
                setting: 'eeg.navigator.relativeTime',
            ...
            },
            {
                component: 'settings-dropdown',
                options: [
                    { label: 'Show only the epoch', value: 'full' },
                    { label: 'Center the epoch with faded context', value: 'centered' },
                ],
                setting: 'eeg.epochMode.display',
                text: 'How to display the current epoch on the page.',
                type: 'setting',
                width: '60%',
            },
            {
                component: 'settings-number',
                setting: 'eeg.epochMode.contextEpochs',
                text: 'Number of faded epochs shown on each side of a centered epoch.',
                min: 1,
                max: 5,
                type: 'setting',
                width: '5rem',
...
export const settings: EegInterfaceSettings = safeObjectFrom({
    _userDefinable: {
        ...
        'epochMode.contextEpochs': Number,
        'epochMode.display': String,
        ...
        'trends.pdbsi.threshold': Number,
        'trends.ratio.displayMode': String,
        'trends.ratio.markCrossing': Boolean,
        'trends.ratio.mirrorMode': Boolean,
        ...
        'navigator.relativeTime': Boolean,
    ...
    trends: {
        aeeg: {
            ...
            // provide colours for any custom derivation ids not mapped to a hemisphere.
            derivationColors: {} as { [id: string]: SettingsColor },
            displayMode: 'separate' as 'separate' | 'superimposed',
        },
        // Ratio + pdBSI display-only settings. Math knobs (epochLength, bands,
        // averageReference, …) live in `CommonBiosignalSettings.trends.{ratio,pdbsi}`.
        ratio: {
            displayMode: 'separate' as 'separate' | 'superimposed',
            markCrossing: true,
            mirrorMode: false,
            showFill: true,
            showThreshold: true,
            threshold: 0.26,    // TAR published cutoff (van Stigt 2023)
        },
        pdbsi: {
            markCrossing: true,
            showFill: true,
            showThreshold: true,
            threshold: 0.52,    // ELECTRA-STROKE delta-band cutoff
        },
    },
    yPadding: 1,
})
